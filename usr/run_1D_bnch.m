...
for reltol = RTOL
    ...
    maxstep         =  nop;                  % maximum number of time steps
    ...
    abstol      = reltol/1e3;                 % absolute residual tolerance for nonlinear iterations
    maxit       = 100;                   % maximum iteration count
    CFL         =  0.50;                % (physical) time stepping courant number (multiplies stable step) [0,1]
    dtmax       = 2.5e-3*yr;              % maximum time step
    ...
    fh15 = figure(15);
    p1 = loglog(reltol,EM,'kd','MarkerSize',8,'LineWidth',2); hold on; box on;
    p2 = loglog(reltol,ES,'rs','MarkerSize',8,'LineWidth',2);
    p3 = loglog(reltol,EXFe,'go','MarkerSize',8,'LineWidth',2);
    p4 = loglog(reltol,EXSi,'bx','MarkerSize',8,'LineWidth',2);
    p5 = loglog(reltol,ECFe,'m.','MarkerSize',8,'LineWidth',2);
    p6 = loglog(reltol,ECSi,'c*','MarkerSize',8,'LineWidth',2);
    ...
    if reltol == RTOL(1)
        p7 = loglog(RTOL,1e-16.*ones(size(RTOL)),'k-' ,'LineWidth',2);  % plot trend for comparison
    end
    if reltol == RTOL(end)
        legend([p1,p2,p3,p4,p5,p6,p7],{'error $M$','error $S$','error $X_{Fe}$','error $X_{Si}$','error $C_{Fe}$','error $C_{Si}$','machine prec.'},'Interpreter','latex','box','on','location','southeast')
    end
    drawnow;
end