...
% equal grid spacing
clear all; close all
RunID           =  'bnchm_dt_bd3i_4phs_new3';               % run identifier
...
dcFe            =  0.01e-3;                 % amplitude of initial random perturbation to iron component
dcSi            =  0.01e-3;                 % amplitude of initial random perturbation to silicate component
...
% store previous iteration (diagnostic)
    ...
    % subplot(6,1,3);
    % plot((maxstep/2+1/2)*dt:dt:(maxstep-1/2)*dt,diff(HST.EFlFe(maxstep/2:maxstep))/dt,'-',LW{:}); hold on; axis tight; box on;
    % ylabel('$\Delta E_FFe$',TX{:},FS{:}); set(gca,TL{:},TS{:},'XTickLabel',[]);
    % subplot(6,1,4);
    % plot((maxstep/2+1/2)*dt:dt:(maxstep-1/2)*dt,diff(HST.EFlSi(maxstep/2:maxstep))/dt,'-',LW{:}); hold on; axis tight; box on;
    % ylabel('$\Delta E_FSi$',TX{:},FS{:}); set(gca,TL{:},TS{:},'XTickLabel',[]);
    % subplot(6,1,5);
    ...
    p3 = loglog(dt,EFlFe,'go','MarkerSize',8,'LineWidth',2);
    p4 = loglog(dt,EFsFe,'bx','MarkerSize',8,'LineWidth',2);
    p5 = loglog(dt,EFlSi,'y+','MarkerSize',8,'LineWidth',2);
    p6 = loglog(dt,EFsSi,'kv','MarkerSize',8,'LineWidth',2);
    p7 = loglog(dt,ECFe,'m.','MarkerSize',8,'LineWidth',2);
    p8 = loglog(dt,ECSi,'c*','MarkerSize',8,'LineWidth',2);
    ...
    if dt == DDT(end)
        legend([p1,p2,p3,p4,p5,p6,p7,p8],{'error $M$','error $S$','error $F^l_{\mathrm{Fe}}$','error $F^s_{\mathrm{Fe}}$',...
            'error $F^l_{\mathrm{Si}}$','error $F^s_{\mathrm{Si}}$','error $C_{\mathrm{Fe}}$','error $C_{\mathrm{Si}}$'},...
