function [xq,cxq,cmq]  =  equilibrium_single(T0,c,P,Tphs0,Tphs1,cphs0,cphs1,perT,perCx,perCm,clap,PhDg,TINY)
...
cx1 = max(0,min(1,          perCx .*erfc((2+PhDg).*(T-perT)./(1-perT))));
cx2 = max(0,min(1, perCx+(1-perCx).*erfc((1+PhDg).*(T     )./   perT) ));

cxq = zeros(size(T));
cxq(T>=perT) = cx1(T>=perT);
cxq(T< perT) = cx2(T< perT);

cm1 = max(0,min(1,          perCm .*erf((1.0+PhDg/10).*(1   -T)./(1-perT))./erf((1.0+PhDg/10))));
cm2 = max(0,min(1, perCm+(1-perCm).*erf((0.9+PhDg/10).*(perT-T)./(  perT))./erf((0.9+PhDg/10))));
