...
%     figure(100);clf
    ...
    while resnorm/resnorm0 >= reltol && resnorm >= abstol && iter <= maxit
        ...
        if ~bnchm
            ...
            % plot(iter,log10(resnorm_TC),'xk','MarkerSize',15); hold on %iter,log10(normS)     ,'.r','MarkerSize',3, ...
            %      plot(iter,log10(normXFe)   ,'+r','MarkerSize',15) ;
            %      plot(iter,log10(normXSi)   ,'+b','MarkerSize',15) ;
            %      plot(iter,log10(normCFe)   ,'xr','MarkerSize',15) ;
            %      plot(iter,log10(normCSi)   ,'xb','MarkerSize',15) ;
            %      plot(iter,log10(normFFe)   ,'*r','MarkerSize',15) ;
